...
Sleep_Jishi_Typedef Sleep_Jishi_Struct;
...
u8 RTC_Init(void)
{
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x5055)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
		...
		RTC_SetPrescaler(16383); //设置RTC预分频的值
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2019,12,2,14,54,1);  //设置时间	
...
{		 
	...
	{
		RTC_Get();//更新时间 
		if(Work_Struct.Mode_type.Weakup.WeakUP_Time>0)
		{
			Work_Struct.Mode_type.Weakup.WeakUP_Time--;
			if(Work_Struct.Mode_type.Weakup.WeakUP_Time==0)
			{
